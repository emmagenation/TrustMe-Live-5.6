...
const FRONTEND = path.join(__dirname, process.env.FRONTEND || 'TrustMe_1_1_4.html');
...
const pub = u => ({ id: u.id, name: u.name, role: u.role, camp: u.camp, batch: u.batch, pic: u.pic || null,
  ...(u.role === 'vendor' ? Object.assign({ cat: u.cat, pkgs: u.pkgs, gallery: u.gallery || [], verified: !!u.verified, callPhone: u.callPhone || u.phone }, stats(u.id)) : {}) });
...
const routes = [
  ...
  ['POST', /^\/api\/auth\/verify$/, c => {
    ...
    if (!u) {
      ...
      if (role === 'vendor') {
        Object.assign(u, { cat: str(b.cat, 30), verified: false, jobs: 0, pkgs: [], gallery: [] });
  ...
  ['POST', /^\/api\/bookings\/(\w+)\/dispute$/, c => {           // the corps member says the finished work is not right
    if (c.u.role !== 'corper') fail(403, 'Only the corps member who booked can open a dispute.');
    const b = db.bookings[c.m[1]]; if (!b || b.corperId !== c.u.id) fail(404, 'Booking not found.');
